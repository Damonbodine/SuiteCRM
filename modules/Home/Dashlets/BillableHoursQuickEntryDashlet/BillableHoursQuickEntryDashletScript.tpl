...
BillableHours.exportToPDF = function(dashletId) {
    // Show PDF export modal
    BillableHours.showPDFExportModal(dashletId);
};
...
BillableHours.showPDFExportModal = function(dashletId) {
    ...
    if (!modal) {
        // Load the modal template via AJAX
        var params = {
            module: 'Home',
            action: 'CallMethodDashlet',
            method: 'getPDFExportModal',
            dashlet_id: dashletId,
            dashlet_class: 'BillableHoursQuickEntryDashlet',
            to_pdf: true
        };
        ...
        YAHOO.util.Connect.asyncRequest('POST', 'index.php', {
            success: function(response) {
                ...
                // Show the modal
                $('#pdf_export_modal').modal('show');
            },
            failure: function() {
                BillableHours.showStatus(dashletId, 'Failed to load PDF export modal', 'error');
            }
        }, BillableHours.buildPostData(params));
    } else {
        // Show existing modal
...
BillableHours.generatePDF = function() {
    ...
    generateBtn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Generating PDF...';
    generateBtn.disabled = true;
    
    // Create a form and submit it to trigger PDF download
    var downloadForm = document.createElement('form');
    downloadForm.method = 'POST';
    downloadForm.action = 'index.php';
    ...
    // Close modal and restore button after a delay
    setTimeout(function() {
        $('#pdf_export_modal').modal('hide');
